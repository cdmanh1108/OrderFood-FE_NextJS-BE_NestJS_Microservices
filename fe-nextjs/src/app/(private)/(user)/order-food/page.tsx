...
const LOCAL_PAYMENT_RETURN_URL = "http://localhost:5000";
...
function isPayosCanceledReturn(
  status: string | null,
...
export default function CustomerMenuPage() {
  ...
  const isPlaceOrderSubmittingRef = useRef(false);
  ...
  useEffect(() => {
    const orderIdFromReturn = searchParams.get("orderId");
    const status = searchParams.get("status");
    const cancelFlag = searchParams.get("cancel");

    if (!orderIdFromReturn || !isPayosCanceledReturn(status, cancelFlag)) {
      return;
    }

    const requestKey = `${orderIdFromReturn}|${status ?? ""}|${cancelFlag ?? ""}`;
    if (handledPayosReturnRef.current === requestKey) {
      return;
    }
    handledPayosReturnRef.current = requestKey;

    let isMounted = true;

    const syncCanceledPayment = async () => {
      try {
        const payment = await paymentApi.getByOrderId(orderIdFromReturn);

        if (payment.status === "PENDING") {
          await paymentApi.cancel(
            payment.id,
            "Khách hàng hủy thanh toán trên PayOS",
          );
        }

        if (isMounted) {
          setSuccess("Bạn đã hủy thanh toán.");
        }
      } catch (error) {
        if (isMounted) {
          setErrorStatus(
        ...
        ["code", "id", "cancel", "status", "orderCode", "orderId"].forEach(
          (key) => cleanedParams.delete(key),
        );
        const queryString = cleanedParams.toString();
        router.replace(
          queryString ? `/order-food?${queryString}` : "/order-food",
        );
      }
    };

    void syncCanceledPayment();

    return () => {
      isMounted = false;
    };
  }, [router, searchParams, setErrorStatus, setSuccess]);
  ...
  const handleSetDefaultAddress = async (addressId: string) => {
    ...
    try {
      ...
      await setAddress(addressId);
      await loadAddresses();
      setSuccess("Đã đặt địa chỉ này làm mặc định và cập nhật cho giỏ hàng");
    } catch (error) {
      setErrorStatus(
        error instanceof Error
          ? error.message
  ...
  const handleCalculateCheckout = async () => {
    ...
    if (!selectedAddress) {
      setErrorStatus("Vui lòng chọn địa chỉ giao hàng trước khi thanh toán");
      return;
    }
    ...
    try {
      ...
      setErrorStatus(message || "Lỗi tính toán hóa đơn");
    } finally {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (isPlaceOrderSubmittingRef.current || isPlacingOrder) {
      return;
    }

    isPlaceOrderSubmittingRef.current = true;
    setIsPlacingOrder(true);
    try {
      ...
      if (gatewayMethod === "PAYOS") {
        ...
        if (!checkoutUrl) {
          throw new Error(
            "Không thể tạo đơn thanh toán PayOS, vui lòng thử lại sau",
          );
        }
      ...
      const message = error instanceof Error ? error.message : undefined;
      setErrorStatus(message || "Đặt hàng thất bại");
    } finally {
      isPlaceOrderSubmittingRef.current = false;
      setIsPlacingOrder(false);
    }
  };
  return (
          ...
          footer={
            <>
              ...
              <Button
                ...
                leftIcon={
                  !isPlacingOrder ? <CreditCard size={16} /> : undefined
                }
              >
            ...
            <div className="space-y-5">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              void removeItem(item.menuItem.id).catch(
                                (error) => {
                                  setErrorStatus(
                                    error instanceof Error
                                      ? error.message
              ...
              <section className="rounded-2xl border border-brand-amber/20 bg-brand-beige/30 p-4">
                <div className="flex items-center justify-between text-sm text-brand-gray-700">
                  <span>Tạm tính</span>
                  <span>{formatCurrency(cartItemsSubtotal)}</span>
                </div>
            ...
            <div className="space-y-4">
              ...
              <section className="rounded-xl border border-brand-amber/20 bg-brand-beige/30 p-4">
                ...
                <div className="flex justify-between font-bold text-base mt-3 pt-3 border-t border-brand-gray-200">
                  ...
                  <span className="text-brand-amber text-xl">
