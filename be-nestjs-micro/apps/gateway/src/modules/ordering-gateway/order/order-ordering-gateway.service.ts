...
export class OrderOrderingGatewayService {
    ...
    return firstValueFrom(
      this.orderingClient
        .send<
          DeleteOrderResult,
          DeleteOrderCommand
        >(ORDERING_PATTERNS.DELETE_ORDER, command)
        .pipe(
          catchError((error) =>
            throwError(() => mapRpcErrorToHttpException(error)),
          ),
  ...
  async findAllUser(userId: string, dto: ListOrdersRequestDto) {
    ...
    const result = await firstValueFrom(
      this.orderingClient
        .send<
          ...
          ListOrdersQuery
        >(ORDERING_PATTERNS.LIST_ORDERS, query)
        .pipe(
          catchError((error) =>
            throwError(() => mapRpcErrorToHttpException(error)),
          ),
    ...
    const result = await firstValueFrom(
      this.orderingClient
        .send<
          OrderDetailResult,
          GetOrderDetailQuery
        >(ORDERING_PATTERNS.GET_ORDER_DETAIL, query)
        .pipe(
          catchError((error) =>
            throwError(() => mapRpcErrorToHttpException(error)),
          ),
  ...
  async updateStatus(
    ...
    return firstValueFrom(
      this.orderingClient
        ...
        >(ORDERING_PATTERNS.UPDATE_ORDER_STATUS, command)
        .pipe(
          catchError((error) =>
            throwError(() => mapRpcErrorToHttpException(error)),
          ),
    ...
    return firstValueFrom(
      this.orderingClient
        .send<
          CancelOrderResult,
          CancelOrderCommand
        >(ORDERING_PATTERNS.CANCEL_ORDER, command)
        .pipe(
          catchError((error) =>
            throwError(() => mapRpcErrorToHttpException(error)),
          ),
