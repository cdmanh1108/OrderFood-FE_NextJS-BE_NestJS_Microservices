...
export class RefundsController {
  ...
  createRefund(
    @Payload() command: CreateRefundCommand,
    @Ctx() context: RmqContext,
  ) {
    return handleRpcMessage(context, () =>
      this.refundsService.createRefund(command),
    );
  }

  @MessagePattern(PAYMENT_PATTERNS.GET_REFUND_BY_ID)
  getRefundById(
    @Payload() command: GetRefundByIdCommand,
    @Ctx() context: RmqContext,
  ) {
    return handleRpcMessage(context, () =>
      this.refundsService.getRefundById(command),
    );
  }

  @MessagePattern(PAYMENT_PATTERNS.GET_REFUNDS_BY_PAYMENT_ID)
  getRefundsByPaymentId(
    @Payload() command: { paymentId: string },
    @Ctx() context: RmqContext,
  ) {
    return handleRpcMessage(context, () =>
      this.refundsService.getRefundsByPaymentId(command),
    );
  }

  @MessagePattern(PAYMENT_PATTERNS.LIST_REFUNDS)
  listRefunds(
    @Payload() command: ListRefundsCommand,
    @Ctx() context: RmqContext,
  ) {
    return handleRpcMessage(context, () =>
      this.refundsService.listRefunds(command),
    );
  }
}
