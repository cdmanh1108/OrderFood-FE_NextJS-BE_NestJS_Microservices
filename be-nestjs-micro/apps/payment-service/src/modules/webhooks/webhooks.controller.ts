...
export class WebhooksController {
  ...
  handleWebhook(
    @Payload() command: HandlePaymentWebhookCommand,
    @Ctx() context: RmqContext,
  ) {
    return handleRpcMessage(context, () =>
      this.webhooksService.handleWebhook(command),
    );
  }

  @MessagePattern(PAYMENT_PATTERNS.LIST_PAYMENT_WEBHOOK_LOGS)
  listWebhookLogs(
    @Payload() command: ListPaymentWebhookLogsCommand,
    @Ctx() context: RmqContext,
  ) {
    return handleRpcMessage(context, () =>
      this.webhooksService.listWebhookLogs(command),
    );
  }

  @MessagePattern(PAYMENT_PATTERNS.CONFIRM_WEBHOOK_URL)
  confirmWebhookUrl(
    @Payload() command: ConfirmWebhookUrlCommand,
    @Ctx() context: RmqContext,
  ) {
    return handleRpcMessage(context, () =>
      this.webhooksService.confirmWebhookUrl(command),
    );
  }
}
